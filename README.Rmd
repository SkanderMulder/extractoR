...
> **Self-Correcting Structured Data Extraction from Text Using Large Language Models**

**extractoR** is an R package that automatically validates and corrects LLM outputs against your schema. Inspired by Python's [instructor](https://github.com/jxnl/instructor) and [outlines](https://github.com/outlines-dev/outlines), it eliminates the pain of malformed JSON and schema violations through intelligent self-correction.

**Stop fighting with LLM outputs. Get guaranteed valid data or controlled failure.**
...
- **Automatic Validation**: JSON Schema validation with detailed error reporting
- **Self-Correction**: Intelligent retry with LLM feedback until success
- **Rich Schema Support**: Enums, nested objects, arrays, optional fields, and complex structures
- **Optional Fields**: Mark fields as optional using `optional()` for flexible extraction
- **Local & Cloud Models**: Works with OpenAI, Anthropic, or local Ollama models
- **Multiple Strategies**: Choose between reflective, direct, or polite correction styles
...
schema <- list(
  ...
  subtitle = optional("character"),
  tags = optional(list("character")),

  # Nested objects
  author = list(
    name = "character",
    email = optional("character")  # Optional nested field
  ),
...
Mark fields as optional when they may not be present in all extractions:
...
schema <- list(
  ...
  # Optional fields
  subtitle = optional("character"),
  doi = optional("character"),
  keywords = optional(list("character")),

  # Optional nested objects
  funding = optional(list(
    agency = "character",
...
By default, all fields are required. Use `optional()` to mark fields that may be missing from the extracted data.
...
extractoR excels at extracting structured information from various domains:
...
4. **Self-Correction**: On failure, LLM receives error details and retries
5. **Guaranteed Output**: Valid data returned or controlled error thrown
...
extractoR works with any LLM supported by [ellmer](https://github.com/hadley/ellmer):
...
1. **Use appropriate models**: Smaller models for simple schemas, larger for complex reasoning
2. **Set temperature = 0.0**: For deterministic, reproducible outputs
3. **Choose the right strategy**: `direct` is faster, `reflect` is more thorough
4. **Limit retries**: 3-5 retries is usually sufficient
5. **Test schemas**: Verify with sample data before production use
6. **Use Ollama for privacy**: Keep sensitive data on your infrastructure
...
- [x] Support for optional fields in schemas
...
**Built with:**
- [ellmer](https://github.com/hadley/ellmer) - R interface to LLMs
- [jsonvalidate](https://github.com/ropensci/jsonvalidate) - JSON Schema validation
- [jsonlite](https://github.com/jeroen/jsonlite) - JSON parsing

**Inspired by:**
- [instructor](https://github.com/jxnl/instructor) - Python structured outputs
- [outlines](https://github.com/outlines-dev/outlines) - Python structured generation
